...
void Register::on_commit_clicked()
{
    ...
    if(ui->head_line_2->text().isEmpty()){
        ...
        headTemp = ui->head_line_2->text();
    randomNumber = QRandomGenerator::global()->bounded(100);
    qDebug() << "randomNumber" << randomNumber;
    DHTemp = static_cast<quint64>(qPow(5, randomNumber)) % 23;
    qDebug() << "DHTemp:" << DHTemp;
    tcpServerConnect();
...
void Register::sendMessage(QString Msg){
    ...
    if(Msg == "register"){
        ...
        QString string = "register|"+nameTemp+"|"+passwordTemp+"|"
                +signTemp+"|"+headTemp+"|"+QString::number(DHTemp);
        QByteArray message;
...
void Register::readMessage(){
    ...
    if(msg[0] == "register"){
        ...
        writeDH(myAccount,QString::number(randomNumber));
    }else{
        qDebug() << "注册失败";
    }
...
void Register::writeDH(QString myAccount, QString key) {
