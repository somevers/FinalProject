...
void PublicChat::readMessage(){
    ...
    qDebug() << "接收方:" << msg[0];//my
    qDebug() << "msg:" << msg[2];
    QString decrypt = PasswordHelper::aesDecryption(msg[2],key,iv);
    if(msg[0] == "init"){
        ...
        ui->chatDisplay->append(tr("%1: %2").arg(msg[1]).arg(decrypt));
...
void PublicChat::sendMessage(){
    QString key("1234567812345678");
    QString iv("1234567812345678");
    QString encrypt = PasswordHelper::aesEncryption(ui->sendMsgLine->text(),key,iv);
    QString decrypt = PasswordHelper::aesDecryption(encrypt,key,iv);
    //string = 接收方 + | + 发送方 + 发送信息
    QString string = "ChatALL:|" + myAccount + "|" + encrypt;
    qDebug() << encrypt;
    QByteArray message;
