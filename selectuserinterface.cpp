...
void SelectUserInterface::readMessage()
{
    ...
    if(msg[0] == "friend"){
        ...
        userList << ui->user1->text() << ui->user2->text() << ui->user3->text()<< ui->user4->text();
        for(int i=1; i+1<msg.length();i++){
            userList[i-1] = msg[i+1];
        }
        ui->user1->setText(userList.value(0));
        ui->user2->setText(userList.value(1));
        ui->user3->setText(userList.value(2));
        ui->user4->setText(userList.value(3));
        }
    }
}
...
void SelectUserInterface::sendMessage(QString Msg)
{
    ...
    if(Msg == "friend"){
        QString string = "friend|" + myAccount;
        qDebug() << "好友:" << string;
        QByteArray message;
        //以只读打开QByteArray，并设置版本，服务端客户端要一致
        QDataStream out(&message,QIODevice::WriteOnly);
...
void SelectUserInterface::on_add_clicked()
{
    if(!newAdd){
        qDebug() << "好友";
        newAdd = new add();
        newAdd->show();
        addFlag = true;
        connect(this,SIGNAL(sendAddAccount(QString)),
                       newAdd,SLOT(getAddAccount(QString)));
        emit sendAddAccount(myAccount);
        connect(newAdd, SIGNAL(sendAddClose(bool)), this, SLOT(getAddClose(bool)));
    }
}

void SelectUserInterface::getAddClose(bool flag){
    if(addFlag == flag)
    {
        qDebug() << "释放好友页面";
        newAdd->deleteLater();
